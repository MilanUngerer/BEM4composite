...
print "Longitud de onda interior:", lam0, "micras"
print "Longitud de onda exterior:", lam, "micras"

########################################

#Importando mallas
grid = bempp.api.import_grid('/home/milanungerer/meshes/45m_45mm/42448_45_45.msh')
...
op_discretizado = op.strong_form()
op_discretizado_cuadrado = op_discretizado * op_discretizado

#RHS
rhs = op_discretizado * np.concatenate([dirichlet_grid_fun.coefficients, alfa*neumann_grid_fun.coefficients])
...
print("Shape of matrix: {0}".format(op_discretizado_cuadrado.shape))
x,info = gmres(op_discretizado_cuadrado, rhs, tol=1e-5, callback = iteration_counter, maxiter = 5000, restart = 5000)
...
Campo_en_antena = (dlp_pot_ext * exterior_field_dirichlet - slp_pot_ext * exterior_field_neumann).ravel() + Amp * np.exp(1j*k0*antena[0])
